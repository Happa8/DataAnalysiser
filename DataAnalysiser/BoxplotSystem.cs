...
namespace DataAnalysiser
{
    public static class BoxplotSystem
    {
        public static double[] setScale(double[] dataarray)
        {
            ...
            {
                maxScale = (Math.Floor(maxValue / Math.Pow(10, maxDigits - 1)) % 10 + 1) * Math.Pow(10, maxDigits - 1);
                minScale = 0;
            }
            ...
            else if(maxValue >= 0 && minValue < 0 && maxValue < minValue)
            {
                ...
                minScale = -1 * (Math.Floor((-1 * minValue) / Math.Pow(10, minDigits - 1)) % 10 - 1) * Math.Pow(10, minDigits - 1);
                maxScale = -1 * minValue;
            }
